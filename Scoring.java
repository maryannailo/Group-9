...
public class Scoring {
    ...
    public static void foxScore()
    {
        String[] fScore = new String[]{
                ...
                "FOX SCORE CARD (B)\n" +
                        "Score points shown for each Fox, depending on the number of unique pairs of the other wildlife types adjacent to it. Other adjacent Fox pairs do not score. Each pair of other wildlife must be uniqure- the same wildlife may not count for more than one pair, even if four or six of that wildlife are adjacent. Pairs of wildlife do not need to be adjacent to each other.",
                "FOX SCORE CARD (C)\n" +
    ...
    public static void elkScore()
    {
        String[] eScore=new String[] {
                ...
                "ELK SCORE CARD (C)\n" +
                        "Score points shown for each group of Elk in each exact shape. Two groups of Elk may be ajacent to one another, howeverm each Elk may only count fora single group. Groups may be rotated."};
    ...
    public static void hawkScore()
    {
        String[] hScore=new String[] {
                ...
                "HAWK SCORE CARD (C)\n" +
                        "Score 3 points for each pair of Hawks that share a direct line of sight between them. Each Hawk may only be counted as part of multiple pairs. Line of sight is not interrupter by a gap ion habitatl tiles, or by other wildlife."};
    ...
    public static void salmonScore()
    {
        String[] sScore=new String[] {
                "SALMON SCORE CARD (A)\n" +
                        "Score poitns shown for each run of Salmon, depending on length of run. A run is defined as a group of adjacent Salmon whhere each Salmon is adjacent to no more than two other Salmon. A gorup of three Salmon in a traingle shape may count as a run, but no other Salmon may be attached to this run. Each run of Salmon may not have any other Salmon adjacent to it.",
                "SALMON SCORE CARD (B)\n" +
                        "Score points shown for each run of Salmon, depending on length of run. A run is defined as a gorup of adjacent Salmon where each Salmon is adjacent to no more than two other Salmon. A group of three salmon in a traingle shape may count as a run, but no other Salmon amy vbe attacked to shi run. Each run of Salmon may not have any other Salmon adjacent to it.",
};

        System.out.println(sScore[random.nextInt(2)]);
    }
